#!/usr/bin/env python3
"""Utility Flask app to visualize enhanced Gemini annotations.

Run with:
    python utils/visualize_gemini_annotations.py --data_dir path/to/enhanced_annotations

The application scans the specified directory for sub-folders produced by
`gemini_enhance.py` (each containing an `enhanced_expressions.json` and the
...
<head>
    <meta charset="utf-8">
    <title>Gemini Annotation Visualizer</title>
    <style>
...
<body>
    <div class="container">
        <h1>Gemini Annotation Visualizer (<code>{{ data_dir }}</code>)</h1>
        {% if not items %}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize enhanced Gemini annotations with a simple Flask app")
    parser.add_argument(
        "--data_dir",
        type=str,
        default="enhanced_annotations",
