...
Extreme position detection provides additional spatial context by identifying instances that occupy boundary locations within their respective object categories. This analysis proves particularly valuable for scenes containing multiple objects of the same type, where conventional grid-based positioning may be insufficient for unique identification. The system determines topmost, bottommost, leftmost, and rightmost instances within each category using margin-based separation criteria, enabling expressions such as ``the leftmost ship'' or ``the topmost building'' when multiple instances of the same class are present. Size relationship analysis complements positional features by determining the largest and smallest objects within each category through area-based comparisons using threshold ratios to ensure significant size differences before assignment.

Spatial relationship calculation establishes relative positioning between objects to enable complex referring expressions that describe objects in terms of their proximity to other instances. The system employs an angle-based directional system spanning eight compass directions (north, northeast, east, southeast, south, southwest, west, northwest), calculating angular relationships between object centroids to determine directional positioning. Distance-based filtering uses dynamic thresholds adjusted according to object sizes to include only meaningful nearby relationships, preventing spurious connections between distant objects. The system handles borderline relationships by recording multiple directional possibilities when objects fall near angular boundaries, ensuring comprehensive coverage of spatial interpretations.

Group formation addresses scenarios where multiple instances of the same object category appear within patches, enabling collective referring expressions and reducing annotation redundancy. The system utilizes DBSCAN clustering with carefully tuned parameters: epsilon distance of 40 pixels, minimum cluster size of 2 objects, and maximum cluster size of 8 objects, operating separately within each object category to maintain semantic coherence. Class-level groups are automatically generated for categories containing multiple instances, creating expressions such as ``all ships in the image'' when numerous instances of the same type are present. Additionally, the system creates special pair groups that combine small and large vehicles when both vehicle types coexist within patches, enabling more natural referring expressions that reference vehicle collections rather than individual instances.
